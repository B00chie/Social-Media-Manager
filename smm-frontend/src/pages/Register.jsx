...
function Register() {
  ...
  const handleRegister = async () => {
    ...
    if (!email) newErrors.email = 'Ovo polje je prazno'
    if (!password) newErrors.password = 'Ovo polje je prazno'
    if (!confirm) newErrors.confirm = 'Ovo polje je prazno'
    if (password && confirm && password !== confirm) newErrors.confirm = 'Lozinke se ne poklapaju'
    ...
    if (res.ok) {
      ...
      setErrors({ username: 'Korisničko ime već postoji' })
    }
  }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#0F0F1A' }}>
      ...
      <div style={{ width: '50%', background: '#1A1A2E', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 50px' }}>
        <div style={{ width: '100%' }}>
          <h2 style={{ color: '#fff', fontSize: '48px', fontWeight: '700', textAlign: 'center', fontFamily: 'Inter', marginBottom: '8px' }}>Kreiraj nalog</h2>
          <p style={{ color: '#A0A0B0', fontSize: '24px', fontWeight: '400', textAlign: 'center', fontFamily: 'Inter', marginBottom: '32px' }}>Pridruži se SMM</p>
          ...
          <label style={{ color: '#fff', fontSize: '16px', fontWeight: '500', display: 'block', marginBottom: '10px', fontFamily: 'Inter' }}>Email</label>
          <input value={email} onChange={e => { setEmail(e.target.value); setErrors(p => ({ ...p, email: null })) }} placeholder="Enter Email" style={inputStyle('email')} />
          <ErrorBubble field="email" />

          <label style={{ color: '#fff', fontSize: '16px', fontWeight: '500', display: 'block', marginBottom: '10px', fontFamily: 'Inter' }}>Password</label>
...
<ErrorBubble field="password" />

<label style={{ color: '#fff', fontSize: '16px', fontWeight: '500', display: 'block', marginBottom: '10px', fontFamily: 'Inter' }}>Confirm password</label>
<div style={{ position: 'relative', width: '100%' }}>
  <input
    ...
    style={{
      width: '100%',
      padding: '18px 44px 18px 20px',
      marginBottom: errors.confirm ? '4px' : '16px',
      background: '#1A1A2E',
      border: errors.confirm ? '1px solid #FF5252' : '1px solid rgba(107,107,107,0.4)',
      borderRadius: '8px', color: '#A0A0B0', fontSize: '16px',
      fontFamily: 'Inter', boxSizing: 'border-box', outline: 'none'
    }}
  />
...
<ErrorBubble field="confirm" />

          <button className='btn-primary' onClick={handleRegister} style={{ width: '100%', padding: '16px 35px', background: '#6C63FF', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: '500', fontFamily: 'Inter', cursor: 'pointer', marginBottom: '16px', marginTop: '8px' }}>
            Registruj se
          </button>

          <p style={{ textAlign: 'center' }}>
            <span onClick={() => navigate('/login')} style={{ color: '#6C63FF', fontSize: '16px', fontWeight: '500', fontFamily: 'Inter', textDecoration: 'underline', cursor: 'pointer' }}>
